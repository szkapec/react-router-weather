...
const api =
  "https://api.openweathermap.org/data/2.5/weather?q=Warszawa&APPID=8c0ca88fe2b8fe6d6c954e8c13f95f86&units=metric";
...
class ViewStart extends React.Component {
  ...
  sprawdz = () => {
    if (this.state.wilgotnosc > 70) {
      ...
      return (
        <div style={{ fill: "red" }} className="ViewStart__img">
    ...
    return ( 
      <div className="ViewStart__contener">
        ...
        <div className="ViewStart__temp">
          <div>
            Temperatura<p>{Math.floor(this.state.temperatura)}&#176;C</p>
          </div>
          <div>
            Ciśnienie <p>{this.state.cisnienie} hPa</p>
          </div>
          <div>
            Wiatr: <p>{Math.floor(this.state.wiatr)} km/s</p>
          </div>
        </div>
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: `repeat(${this.state.wiatr > 20 ? 3 : 2},1fr)`
          }}
